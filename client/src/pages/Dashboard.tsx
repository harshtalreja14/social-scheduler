...
const Dashboard = () => {
  ...
  useEffect(() =>{
    const fetchDashboardData = async () => {
      try {
        const [postRes, accountsRes, activityRes] = await Promise.all([api.get("/api/post"), api.get("/api/accounts"),api.get("/api/activity")])
        ...
        console.error("Error feteching dashboard data", error);
      }
    };
    fetchDashboardData()
  }, [])
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {statCards.map((card) =>(
          <div key={card.label} className="bg-white hover:bg-red-50 relative border border-slate-200 rounded-2xl p-5 hover:bordered200 transition-all">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          ...
          <span className="text-sm text-slate-400">{activities.length}</span>
        </div>
        {activities.length === 0 ? (
          ...
          <div className="divide-y divide-slate-50">
            {activities.map((activity) =>(
              <div key={activity.id} className="flex items-start gap-4 px-6 py-4 hover:bg-sky-50/50 transition-colors">
