    ...
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        ...
        <div className="relative group mt-12">
          <textarea className="w-full px-6 py-6 bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 outline-none focus:border-slate-400 transition resize-none h-40" placeholder="Share your idea... (e.g. A post about the launch of out new eco-friendly coffee beans)" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
            <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
              ...
              <button className="bg-slate-900 hover:bg-slate-800 text-white flex items-center gap-2 px-4 py-2 rounded-lg" onClick={handleGenerate} disabled={loading}>
                {loading ? (
                  <>
        ...
        <div className="flex flex-wrap justify-center gap-2">
            {tones.map((t) => (
              <button className={`px-4 py-1.5 rounded-full text-sm transition-all border ${tone === t ? "bg-red-500 border-red-500 text-white" : "bg-white border-slate-200 text-slate-500 hover:border-slate-300"}`} key={t} onClick={() => setTone(t)}>
                {t}
              </button>
            ))}
        </div>
      ...
      {/* AI Generated Posts */}
      <div className="space-y-6 p-12 border-t border-slate-100">
              ...
              <div key={gen._id} className="group bg-white rounded-2xl border border-slate-100 p-5 hover:border-red-200 transition-all relative overflow-hidden">
                <div className="flex flex-col h-full space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-400 uppercase tracking-widest whitespace-nowrap">{new Date(gen.createdAt).toLocaleString()}</span>
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <button className="flex-1 bg-slate-100 hover:bg-red-500 hover:text-white text-slate-600 text-2xs py-2.5 rounded-lg transition-all" onClick={() => setActiveScheduler(gen)}>
                      Schedule Post
                    </button>
                  </div>
              ...
              generations.length === 0 && (
                <div className="col-span-full flex flex-col items-center justify-center py-16 gap-4">
                  ...
                  <div className="text-center space-y-1">
                    ...
                    <p className="text-sm text-slate-400">Describe your idea above and hit Generate to get started.</p>
                  </div>
                </div>
              )
            }
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-xs text-slate-600 uppercase tracking-widest mb-4">Platforms</label>
                    <div className="flex flex-wrap gap-2">
                      {PLATFORMS.map((p) => {
                        const active = selectedPlatforms.includes(p.id);
                        return (
                          <button key={p.id} type="button" onClick={() => setSelectedPlatforms((prev: string[]) => prev.includes(p.id) ? prev.filter((id) => id !== p.id) : [...prev, p.id])} className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm transition-all ${ active ? "bg-red-50 border-red-300 text-red-500" : "border-slate-200 text-slate-500 hover:border-slate-300"}` }>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs text-slate-600 uppercase tracking-widest mb-2">Date</label>
                    <div className="relative">
                      <CalendarIcon className="size-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                      <input type="date" className="w-full pl-9 pr-3 py-2.5 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm outline-none" value={scheduledDate} onChange={(e) => setScheduledDate(e.target.value)} />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs text-slate-600 uppercase tracking-widest mb-2">Time</label>
                    <div className="relative">
                      <ClockIcon className="size-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                      <input type="time" className="w-full pl-9 pr-3 py-2.5 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm outline-none" value={scheduledTime} onChange={(e) => setScheduledTime(e.target.value)} />
                    </div>
                  </div>
                </div>
