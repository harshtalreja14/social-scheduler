    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                ...
                <button className="p-2 rounded-full hover:bg-slate-100 text-slate-500 transition-colors" onClick={onClose}>
            ...
            <div className="p-4 flex flex-col gap-1">
                {PLATFORMS.map((p) =>{
                    ...
                    return (
                        <button key={p.id} onClick={() => !isConnected && onConnect(p.id)} className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl text-left transition-colors ${isConnected ? "opacity-60 cursor-default" : "hover:bg-slate-50 cursor-pointer"}`}>
                            {/* Icon */}
